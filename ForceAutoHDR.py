...
def add_exe_to_reg():
    ...
    try:
        ...
        (winreg.CreateKey
         (winreg.HKEY_LOCAL_MACHINE, r"Software\Microsoft\Direct3D\Name"))
...
def show_Keys():
    ...
    try:
        ...
        messagebox.showerror(title="Error", message=f"Error accessing registry: {e}")
        keys = []
    return keys
def is_admin():
    try:
...
def vcache_delete_registry_key(key_name):
    ...
    i = messagebox.askyesno(title="Warning.",
                            message=f"Are you sure you want to delete key in path: \n {path}\{key_name}")
    if i == False:
        return
    else:
        try:
            ...
            messagebox.showerror(title="Error",message=f"Registry path not found: {path}\{key_name}")
